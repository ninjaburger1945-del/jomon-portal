...
export default function PageContent({
  ...
  useEffect(() => {
    if (facilitiesData.length > 0) {
      ...
      const activeAndUpcoming = eventsData.filter((e) => {
    ...
    <>
        ...
        {upcomingEventCount > 0 && currentEvent && (
          ...
          >
            <div style={{ marginBottom: "16px" }}>
              ...
              >
                <div>
                  📅 <strong>{currentEvent.date_start}</strong>
        ...
        <section className={`${styles.section} container`}>
          ...
          <div className={styles.tagChipsWrapper}>
            <div className={styles.tagChips}>
              ...
              {allTags.map((tag) => (
                <button
                  key={tag}
                  className={`${styles.tagChip} ${PRIORITY_TAGS.has(tag) ? styles.tagChipPriority : ""} ${
                    selectedType === tag ? styles.tagChipActive : ""
                  }`}
                ...
                >
                  <Link href={`/facility/${facility.id}`} className={styles.card}>
                    <div className={styles.cardImageWrapper}>
                      ...
                      />
                      {facility.id === newestFacilityId && <span className={styles.newBadge}>NEW</span>}
                    ...
                    <div className={styles.cardContent}>
                      <div className={styles.cardMeta}>
                        <span
                          ...
                          style={{ backgroundColor: REGION_COLORS[mapRegionToDisplay(facility.region)] ?? "#666" }}
                        >
                      ...
                      {facility.copy && <p className={styles.cardText}>{(facility.copy as string).slice(0, 14)}</p>}
                    </div>
                  </Link>
                </div>
              );
          ...
          {visibleCount < filteredAndSortedFacilities.length && (
            <div className={styles.loadMoreContainer}>
              <button className={styles.loadMoreBtn} onClick={() => setVisibleCount((prev) => prev + 20)}>
                もっと見る
              </button>
            </div>
          )}
      ...
      {showFloating && (
        <div className={styles.floatingCounter}>
          全 {filteredAndSortedFacilities.length} 件中 <strong>{firstVisibleIndex + 1}</strong> 番目付近を表示中
        </div>
      )}
    </>
  );
