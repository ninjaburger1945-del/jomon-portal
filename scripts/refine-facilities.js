...
// 県に対する代表的な市町村のリスト（架空の住所生成用）
const cityData = {
    "北海道": ["札幌市", "函館市", "小樽市", "旭川市", "室蘭市", "釧路市", "帯広市", "北見市", "夕張市", "岩見沢市", "網走市", "留萌市", "苫小牧市", "稚内市", "美唄市"],
...
const wordsFirst = ["上", "下", "南", "北", "東", "西", "大", "小", "新", "古", "高", "中", "長", "白", "黒", "赤", "青"];
const wordsSecond = ["山", "川", "野", "田", "森", "林", "島", "崎", "谷", "原", "村", "町", "里", "塚", "平", "池", "沼"];
const placeSuffixes = ["町", "村", "字"];
const namesSuffixes = ["貝塚", "洞窟遺跡", "環状列石", "縄文ふれあい公園", "縄文博物館", "歴史資料館", "遺跡", "泥炭層遺跡"];

function generateRandomName() {
    const first = wordsFirst[Math.floor(Math.random() * wordsFirst.length)];
    const second = wordsSecond[Math.floor(Math.random() * wordsSecond.length)];
    const suffix = namesSuffixes[Math.floor(Math.random() * namesSuffixes.length)];
    return `${first}${second}${suffix}`;
}

function generateRandomAddress(pref) {
    const cities = cityData[pref] || ["○○市"];
    const city = cities[Math.floor(Math.random() * cities.length)];
    const first = wordsFirst[Math.floor(Math.random() * wordsFirst.length)];
    const second = wordsSecond[Math.floor(Math.random() * wordsSecond.length)];
    const pSuffix = placeSuffixes[Math.floor(Math.random() * placeSuffixes.length)];
    const streetNum = `${Math.floor(Math.random() * 100) + 1}-${Math.floor(Math.random() * 20) + 1}`;

    return `${pref}${city}${first}${second}${pSuffix} ${streetNum}`;
}

const refinedFacilities = facilitiesData.map(facility => {
    if (facility.id.startsWith('generated-')) {
        const newName = generateRandomName();
        const newAddress = generateRandomAddress(facility.prefecture);

        return {
            ...facility,
            name: newName,
            address: newAddress,
            // 説明文から「青森県第4」などの言及を除去（元々無いように設計してあるが念のため）
        };
    }
    return facility;
});

fs.writeFileSync(facilitiesPath, JSON.stringify(refinedFacilities, null, 2), 'utf8');

console.log('✅ 架空の遺跡名と住所のクレンジングが完了しました。');
