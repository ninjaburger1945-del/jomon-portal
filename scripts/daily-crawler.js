...
【🔍 重要：「実際に存在し、アクセス可能な」URLのみを返してください】
あなたのタスク：
1. 施設名で Google 検索した場合の上位結果を推定
2. **検証ポイント**：候補URLは「実際に存在するドメイン」「削除/404になっていない」「施設の正式情報ページ」であることを確認
3. 疑わしいURLは候補に入れない（ハルシネーション厳禁）
4. 候補URL数は質を優先：信頼できるものだけを2～3個に厳選
5. 候補が不足するなら、関連キーワード（「〇〇遺跡 公式」「〇〇博物館 自治体」など）で検索結果を再推定
